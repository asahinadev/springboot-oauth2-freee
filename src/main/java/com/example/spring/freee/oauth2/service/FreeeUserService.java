package com.example.spring.freee.oauth2.service;
...
import com.example.spring.freee.oauth2.interceptor.LoggingClientHttpRequestInterceptor;
import com.example.spring.freee.oauth2.user.FreeeUser;

import lombok.extern.slf4j.Slf4j;

@Slf4j
public class FreeeUserService
		implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	final String INVALID_USER_INFO_RESPONSE_ERROR_CODE = "invalid_user_info_response";

	FreeeUserRequestEntityConverter requestEntityConverter;
	 ...
	 *                        Registration Id}
	 */
	public FreeeUserService() {
		restTemplate = new RestTemplate();
		restTemplate.setErrorHandler(new OAuth2ErrorResponseErrorHandler());
		restTemplate.setInterceptors(Arrays.asList(
				new LoggingClientHttpRequestInterceptor()));
		requestEntityConverter = new FreeeUserRequestEntityConverter();
	}
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
		...
		if (!Objects.equals(registrationId, "freee")) {
			return null;
		}
		...
		try {
			...
			response = this.restTemplate.exchange(request, FreeeUser.class);
	...
	class FreeeUserRequestEntityConverter
